...
func NewStatsCollector(
	...
	currentTransactionStatementStats := make([]*bufferedStmtStats, 0, 1)

	return &StatsCollector{
		flushTarget:                appStats,
		stmtBuf:                    currentTransactionStatementStats,
...
func (s *StatsCollector) EndTransaction(
	...
	for i := range s.stmtBuf {
		s.stmtBuf[i].key.TransactionFingerprintID = transactionFingerprintID
		s.stmtBuf[i].key.ImplicitTxn = implicitTxn
		if err := s.flushTarget.RecordStatement(ctx, s.stmtBuf[i].key, s.stmtBuf[i].value); err != nil {
			discardedStats++
		}
	}
