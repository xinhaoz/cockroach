...
func (s *Container) RecordStatement(
	...
	stats, created, throttled := s.tryCreateStatsForStmtWithKey(stmtKey, invalidStmtFingerprintID)
	...
	if created {
		...
		estimatedMemoryAllocBytes := stats.sizeUnsafeLocked() + stmtKey.size() + 8

		// We also account for the memory used for s.sampledStatementCache.
		// timestamp size + key size + hash.
		estimatedMemoryAllocBytes += timestampSize + stmtKey.sampledPlanKey.size() + 8
		s.mu.Lock()
		defer s.mu.Unlock()
